...
export default function playlists(state = INITIAL_STATE, action) {
  switch (action.type) {
    case Types.LOAD:
      return {
        ...state,
        currentSound: action.payload.song,
    ...
    case Types.NEXT: {
      const index = state.list.findIndex(song => song.id === state.currentSound.id);
      const next = state.list[index + 1];

      if (next) {
        return { ...state, currentSound: next, position: 0 };
      }
      return state;
    }
    case Types.PREV: {
      ...
      if (prev) {
        return { ...state, currentSound: prev, position: 0 };
      }
      return state;
    }
    case Types.PLAYING:
